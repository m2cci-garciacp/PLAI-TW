...
function createRandomFace() {
    ...
    let cx = R_MAX + Math.random() * (canvas.width-R_MAX);
    let cy = R_MAX + Math.random() * (canvas.height-R_MAX);
    ...
    let face = new Face(canvas, cx, cy, r, {fill:colorFill, line:colorLine}, vx, vy)
    return face ;
}
