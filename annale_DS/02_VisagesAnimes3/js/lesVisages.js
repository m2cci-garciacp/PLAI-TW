...
/**
 * permet de créer un Visage dont le type (Visage 'normal' ou 'Visage Expressif') et les
 * caractéristiques (centre, rayon, déplacement élementaire) sont définies de manière aléatoire
 *
...
function creerVisage(canvas, rMin, rMax, vMin, vMax) {
	...
	switch (getRandomInt(0, 2)) {
		case 0 : return new Visage(canvas, xCentre, yCentre, rayon, vx, vy);
		case 1 : return new VisageExpressif(canvas, xCentre, yCentre, rayon, vx, vy, getRandomInt(0, 2));
		case 2 : return new VisageClown(canvas, xCentre, yCentre, rayon, vx, vy, getRandomInt(0, 2), getRandomInt(1, 3)*rayon/10);
	}
...
const tabVisages = [
	...
	new VisageExpressif(canvas, canvas.width / 2 + 60, canvas.height / 2, 20, -3, 2), // déplacement vx = -3, vy = 2
];
...
document.querySelector("#visagesColor").addEventListener("change", function () {
	Visage.couleurVisage = document.getElementById("visagesColor").value;
